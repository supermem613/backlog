...
const item = addItem("loop-session", "run controlled item", false, queueId);
db.prepare("UPDATE items SET status = ?, priority = ? WHERE id = ?").run("approved", 5, item.id);
...
assertEqual(db.prepare("SELECT status FROM items WHERE id = ?").get(item.id).status, "needs_review", "item waits for queue review after completion");
assertEqual(store.getLoopState(queueId).status, "needs_review", "loop pauses at review gate");
...
assertEqual(store.getLoopState(queueId).status, "stopped", "controller stops cleanly");
